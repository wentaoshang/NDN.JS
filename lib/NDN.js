...
NDN.findIdenticalInterest = function (name) {
    for (var i = 0; i < NDN.PITTable.length; i++) {
	if (NDN.PITTable[i].interest.name.equals(name)) {
	    //XXX: different selectors are ignored for now
	    return NDN.PITTable[i];
        }
    }
...
NDN.prototype.expressInterest = function (name, template, onData, onTimeOut) {
    ...
    var entry = NDN.findIdenticalInterest(name);
...
NDN.prototype.onMessage = function(msg) {
	...
	if (entry != null && entry.closure != null && entry.closure.onInterest != null) {
	    entry.closure[i].onInterest(interest);
	}
